use crate::{response, Command, Environment, GroundSort, Pattern, Sort, SortedWith, Term};
...
impl<T: Typable> Typed<T> {
	...
	pub fn type_decoration(
		...
		let type_ref =
			checker.new_attached_type_variable(self.span(), &mut self.typ as *mut Option<_>);
...
impl<S: Sort> TypeChecker<S> {
	...
	pub fn assert_equal<A: Into<TypeRef<S>>, B: Into<TypeRef<S>>>(
		...
		self.constraints
			.push((expected.into(), given.into(), self.location()))
	}
	...
	pub fn representant(&self, id: usize) -> (usize, TypeRef<S>) {
		match &self.knowledge[id] {
			TypeRef::Var(next, _) if *next != id => self.representant(*next),
			ty => (id, ty.clone()),
		}
	}
	...
	pub fn redirected(&self, ty: &TypeRef<S>) -> TypeRef<S> {
		match ty {
			TypeRef::Var(id, span) => match &self.knowledge[*id] {
				TypeRef::Var(next, _) if next != id => self.redirected(&TypeRef::Var(*next, *span)),
				TypeRef::Var(_, _) => ty.clone(),
				ground => self.redirected(ground),
			},
			TypeRef::Ground(g) => TypeRef::Ground(GroundTypeRef {
				sort: g.sort.clone(),
	...
	pub fn unify(
		...
		root: bool,
		span: Span,
	) -> std::result::Result<(TypeRef<S>, Vec<Constraint<S>>), SortMissmatch<S>> {
						...
						match self.unify(expected_ty, given_ty, true, span) {
							...
							Ok((union, mut new_constraints)) => {
								if expected_rep < given_rep {
									...
									self.knowledge[given_rep] =
										TypeRef::Var(expected_rep, given_span);
								} else {
									self.knowledge[given_rep] = union;
									self.knowledge[expected_rep] =
										TypeRef::Var(given_rep, expected_span);
								}
	...
	pub unsafe fn apply(&self) -> Result<(), S> {
		for (id, ptr) in &self.pointers {
			...
			match self.redirected(&TypeRef::Var(*id, span)).into_ground_sort() {
				...
				Err(_) => return Err(Error::Ambiguity.at(span)),
			}
		}
...
impl<E: Environment> Typable for Term<E> {
	...
	fn type_decoration(
		&mut self,
		checker: &mut TypeChecker<E::Sort>,
		env: &E,
		this_type: TypeRef<E::Sort>,
	) {
		use Term::*;
		match self {
			Const(cst) => {
...
impl<E: Environment> Typable for Pattern<E> {
	...
	fn type_decoration(
		&mut self,
		_checker: &mut TypeChecker<E::Sort>,
		_env: &E,
		_this_type: TypeRef<E::Sort>,
	) {
